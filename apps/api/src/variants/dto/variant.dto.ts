...
export class CreateVariantDto {
    ...
    @ApiPropertyOptional({ example: 'SKU-RED-M', description: 'Unique SKU for this variant' })
    @IsString()
    @IsOptional()
    sku?: string;
    ...
    @ApiPropertyOptional({
        example: 5.0,
        description: 'Price difference from base product price (positive adds, negative subtracts)',
    })
    @IsNumber()
    @IsOptional()
    priceDiff?: number;
