...
export class ProductsController {
    ...
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Create a product', description: 'Create a new product. Requires authentication (Admin role recommended).' })
    ...
    @Get()
    @ApiOperation({ summary: 'List all products', description: 'Retrieve a paginated list of all products with optional filters.' })
    ...
    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Update a product', description: 'Update product details. Requires authentication.' })
    @ApiParam({ name: 'id', description: 'Product ID', example: 'clx_product_id_123' })
    ...
    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Delete a product', description: 'Permanently delete a product. Requires authentication.' })
    @ApiParam({ name: 'id', description: 'Product ID', example: 'clx_product_id_123' })
