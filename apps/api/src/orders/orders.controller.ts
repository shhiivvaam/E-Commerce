...
export class OrdersController {
    ...
    @Post()
    @ApiOperation({ summary: 'Create an order', description: 'Create a new order for the authenticated user.' })
    ...
    @Get()
    @ApiOperation({ summary: 'Get my orders', description: 'Retrieve all orders for the currently authenticated user.' })
    ...
    @Get(':id')
    @ApiOperation({ summary: 'Get an order by ID', description: 'Retrieve a specific order. Users can only retrieve their own orders.' })
    ...
    @ApiUnauthorizedResponse({ description: 'JWT token is missing or invalid' })
    @ApiNotFoundResponse({ description: 'Order not found or does not belong to the user' })
    findOne(@Request() req: any, @Param('id') id: string) {
        return this.ordersService.findOne(id, req.user.userId || req.user.id);
    }

    @Patch(':id/status')
    @ApiOperation({ summary: 'Update order status', description: 'Update the status of an order. Typically restricted to Admin roles.' })
    @ApiParam({ name: 'id', description: 'Order ID', example: 'clx_order_id_123' })
